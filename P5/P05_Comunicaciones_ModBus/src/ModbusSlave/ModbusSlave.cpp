...
enum { 
	FC_READ_COILS   =	0x01,	//Lectura de se�ales discretas de salida
	...
	FC_WRITE_COIL   =   0x05,	//Escritura de una se�al discreta
...
/*
		...
		if (regs_num >= N_ARegs){
			//sprintf(msg,"regs_num : %d --> N_ARegs : %d", regs_num, N_ARegs);
			//Serial.println(msg);
...
int ModbusSlave::read_discrete_registers(unsigned int Func, unsigned int start_addr, unsigned char reg_count, uint16_t *regs)
{
	...
	packet[2] 		= 1;             //S�lo enviaremos un m�ximo de 8 registros;
...
//		return read_discrete_registers(FC_READ_COILS, start_addr,query[REGS_L],C_Regs);
	...
	case FC_READ_REG_DI:
//		return read_discrete_registers(FC_READ_REG_DI, start_addr,query[REGS_L],D_Regs);
		break;
