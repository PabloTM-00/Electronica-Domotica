	...
	public void UpdateElements(final ModBusEvent e) {
		...
		case Const_Modbus.READ_INPUT_DISCRETES:
			...
			for (int i = addr; i < (addr + nReg); i++) {
				...
				// Variable para detectar si el botón está pulsado
				boolean pulsado = (e.getRegs()[i] != mbDIC.getDefaultValue());
				
				switch (mbDIC) {
				case MB_BTN1:
					ledBtn1.setLedOn(pulsado);
					// APARTADO IV: Apagar el TRIAC si se pulsa el BTN1
					if (pulsado) {
						buildModBus(address, Const_Modbus.WRITE_COIL, MB_Registers.MB_Discrete_Output_Coils.MB_TRIAC.getReg(), 0, Cregs);
					}
					break;
				case MB_BTN2:
					ledBtn2.setLedOn(pulsado);
					// APARTADO IV: Apagar el TRIAC si se pulsa el BTN2
					if (pulsado) {
						buildModBus(address, Const_Modbus.WRITE_COIL, MB_Registers.MB_Discrete_Output_Coils.MB_TRIAC.getReg(), 0, Cregs);
					}
					break;
				case MB_OPT:
					ledBtnOpt.setLedOn(pulsado);
					break;
				case MB_PIR:
		...
		case 4:
			if (e.getRegs() != null && e.getRegs().length >= 5) {
				...
				// Mapeo básico a los visores de la interfaz
				gaugePot1.setValue(e.getRegs()[0]);
				gaugePot2.setValue(e.getRegs()[1]);

				int ldrVal = e.getRegs()[2]; // Fotorresistencia
				int fttVal = e.getRegs()[4]; // Fototransistor
				
				gaugeFotor.setValue(ldrVal);
				gaugeFotot.setValue(fttVal);

				// Conversión matemática del sensor TMP36
				...
				// SOLUCIÓN A LA IMAGEN: Forzamos la salida continua de datos por la consola de Eclipse
				System.out.println(String.format("TMP36 RAW: %d -> Voltaje: %.2f V -> Temp: %.2f ºC | LDR: %d | FTT: %d", 
						rawTemp, voltaje, celsius, ldrVal, fttVal));

				// APARTADO II: Lógica de iluminación de la Farola (Fotorresistencia -> Relé)
				if (cbActNRC.isSelected()) {
					try {
						int high = Integer.parseInt(txtHighLevel.getText());
						int low = Integer.parseInt(txtLowLevel.getText());
						
						// Si la iluminación natural cae por debajo del mínimo, encendemos el Relé
						if (ldrVal < low) {
							buildModBus(address, Const_Modbus.WRITE_COIL, MB_Registers.MB_Discrete_Output_Coils.MB_RELE.getReg(), 1, Cregs);
						} 
						// Si vuelve a haber luz suficiente (supera el nivel alto), lo apagamos
						else if (ldrVal > high) {
							buildModBus(address, Const_Modbus.WRITE_COIL, MB_Registers.MB_Discrete_Output_Coils.MB_RELE.getReg(), 0, Cregs);
						}
					} catch (NumberFormatException ex) {
						// Evita cuelgues si borras el texto para escribir otro número
					}
				}

				// APARTADO III: Enclavamiento de Seguridad (Fototransistor -> TRIAC)
				if (cbActDetec.isSelected()) {
					try {
						int detec = Integer.parseInt(txtNivelDetec.getText());
						
						// Si la luz en el fototransistor es inferior al nivel programado, se enciende el TRIAC
						if (fttVal < detec) {
							buildModBus(address, Const_Modbus.WRITE_COIL, MB_Registers.MB_Discrete_Output_Coils.MB_TRIAC.getReg(), 1, Cregs);
						}
						// Al no haber un 'else' apagándolo, el TRIAC se queda enclavado de forma automática
					} catch (NumberFormatException ex) {}
				}
			}
			break;
