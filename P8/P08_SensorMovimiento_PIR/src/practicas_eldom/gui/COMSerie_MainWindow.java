...
public class COMSerie_MainWindow {
	...
	public COMSerie_MainWindow() {	
		...
		JRadioButtonMenuItem P4_Pulsadores = new JRadioButtonMenuItem("2.- Pulsadores");
		P4_Pulsadores.addActionListener(Select_Practica);
		mnPrctica_4.add(P4_Pulsadores);
		
		JRadioButtonMenuItem P4_Interruptor = new JRadioButtonMenuItem("3.- Interruptor");
		P4_Interruptor.addActionListener(Select_Practica);
		mnPrctica_4.add(P4_Interruptor);
		
		JRadioButtonMenuItem P4_Conmutador_int = new JRadioButtonMenuItem("4.- Conmutador - Btn_Opt Interruptor");
		P4_Conmutador_int.addActionListener(Select_Practica);
		mnPrctica_4.add(P4_Conmutador_int);
		
		JRadioButtonMenuItem P4_Conmutador_2sal = new JRadioButtonMenuItem("5.- Conmutador - dos salidas");
		P4_Conmutador_2sal.addActionListener(Select_Practica);
		mnPrctica_4.add(P4_Conmutador_2sal);
		...
		JRadioButtonMenuItem P07_PIR = new JRadioButtonMenuItem("Interruptor Temporizado");
		P07_PIR.addActionListener(Select_Practica);
		mnPrctica_7.add(P07_PIR);
	...
	private void Modbus_Regular_Call(){
		timer = new Timer (ConstantesApp.MS_REGULAR_CALL, new ActionListener ()
		{
			public void actionPerformed(ActionEvent e)
		    {	
	...
	private void handle_Menu(ActionEvent arg0){
		...
		for (int i = 0, n = practicaSel.size(); i < n; i++) {
			if(practicaSel.get(i).equals(Source)) {
				...
				if(MB_Registers.SELPRACT[i]<0x50) {
					timer.stop();
				}else timer.restart();
			}
		}
