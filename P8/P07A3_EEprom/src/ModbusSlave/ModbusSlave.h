...
typedef enum{
	BANK_COILS,
	BANK_DISCRETE_INPUTS,
	BANK_HOLDING_REGS,
	BANK_INPUT_REGS // nuevo banco de input registers
}BankType;
...
class ModbusSlave {
	...
	public:
		...
		int read_input_registers(unsigned int start_addr, unsigned char reg_count, uint16_t *regs);
		...
		void set_BankRegisters(uint16_t *Regs, uint16_t lengthRegs, BankType type);
	};
