 ...
 */
void ModbusSlave::set_BankRegisters(uint16_t *Regs, uint16_t lengthRegs, BankType type) {
	switch(type) {
		case BANK_COILS:
			CRegs = Regs;
			N_CRegs = lengthRegs;
			break;
		case BANK_DISCRETE_INPUTS:
			DRegs = Regs;
			N_DRegs = lengthRegs;
			break;
		case BANK_HOLDING_REGS:
			ARegs = Regs;
			N_ARegs = lengthRegs;
			break;
...
{
	unsigned char function = FC_READ_INPUT_REGS; 	// el codigo es 0x04
	int packet_size = 3;
	int status;
	unsigned int i;
	unsigned char packet[MAX_MESSAGE_LENGTH];
...
//		return preset_multiple_registers(start_addr,query[REGS_L],query,A_Regs);
	...
	case FC_WRITE_REG:
		write_single_register(start_addr,query,ARegs);
		break;
