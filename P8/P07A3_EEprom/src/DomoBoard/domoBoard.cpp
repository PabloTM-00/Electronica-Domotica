...
DomoBoard::DomoBoard()
{
	...
	// configurar potenciometros, analogicos por lo que devuelven un rango de valores (0-1023)
	POT1.pin = POT1_P;
	POT1.valor = analogRead(POT1_P);
	POT1.valor_Df = POT1.valor;
	POT1.Activo = true;

	// se marca como "analogico" para que use analogread y no digitalread (lee la tension 0-5V)
