...
public class DomoBoardGui extends JPanel implements Visualizer {
	...
	public DomoBoardGui(String category, int address, CommTransport sn_Transport) {
		...
		panelSensores.setBorder(new LineBorder(Color.MAGENTA, 2));
		panelSensores.setBounds(10, 265, 570, 120); 
		panelSensores.setLayout(null);
		add(panelSensores);
		
		JLabel lblSensores = new JLabel("Sensores Analógicos (P9)");
		lblSensores.setFont(new Font("Times New Roman", Font.BOLD, 16));
		lblSensores.setBounds(10, 5, 250, 20);
		panelSensores.add(lblSensores);
		
		gaugeFotor = new Radial2Top();
		gaugeFotor.setTitle("LDR (Luz)");
		gaugeFotor.setMaxValue(1023); 
		gaugeFotor.setBounds(10, 25, 170, 80);
		panelSensores.add(gaugeFotor);

		// visor temp de -40 a 125 C
		gaugeTemp = new Radial2Top();
		gaugeTemp.setTitle("Temperatura");
		gaugeTemp.setUnitString("ºC");
		gaugeTemp.setMinValue(-40);
		gaugeTemp.setMaxValue(125); 
		gaugeTemp.setBounds(190, 25, 170, 80);
		panelSensores.add(gaugeTemp);

		gaugeFotot = new Radial2Top();
		gaugeFotot.setTitle("Fototransistor");
		gaugeFotot.setMaxValue(1023); 
		gaugeFotot.setBounds(370, 25, 170, 80);
		panelSensores.add(gaugeFotot);
